...
girar(msg_cmd_vel)
tolerancia= 1.5
if (rango3-tolerancia>0):
    ancho_derecha=rango3-tolerancia
else:
    ancho_derecha=0


##giro de 180 para mirar atras del robot
girar(msg_cmd_vel)
girar(msg_cmd_vel)
if (rango3-tolerancia>0):
    ancho_izquierda=rango3-tolerancia
else:
    ancho_izquierda=0

##movimiento hacia la izquierda cero
objetivo(goal,0,ancho_izquierda)
rospy.sleep(ancho_izquierda*3.3333)
girar_completo(msg_cmd_vel)

if (rango1-tolerancia>0):
    alto_arriba=rango1-tolerancia
else:
    alto_arriba=0

if (rango2-tolerancia>0):
    alto_abajo=rango2-tolerancia
else:
    alto_abajo=0
#movimiento izquierda arriba
objetivo(goal,alto_arriba,ancho_izquierda)
rospy.sleep(alto_arriba*3.3333)
girar_completo(msg_cmd_vel)

#movimiento izquierda abajo
objetivo(goal,-1*alto_abajo,ancho_izquierda)
rospy.sleep((alto_abajo+alto_arriba)*3.3333)
girar_completo(msg_cmd_vel)

#movimiento de regreso a la izquierda cero
objetivo(goal,0,ancho_izquierda)
rospy.sleep(alto_abajo*3.3333)


#movimiento hacia la derecha cero
objetivo(goal,0,-1*ancho_derecha)
rospy.sleep((ancho_derecha+ancho_izquierda)*5)
girar_completo(msg_cmd_vel)



if (rango2-tolerancia>0):
    alto_arriba=rango2-tolerancia
else:
    alto_arriba=0

if (rango1-tolerancia>0):
    alto_abajo=rango1-tolerancia
else:
    alto_abajo=0

#movimiento derecha arriba
objetivo(goal,alto_arriba,-1*ancho_derecha)
rospy.sleep(alto_arriba*3.3333)
girar_completo(msg_cmd_vel)

#movimiento derecha abajo
objetivo(goal,-1*alto_abajo,-1*ancho_derecha)
rospy.sleep((alto_arriba+alto_abajo)*3.3333)
girar_completo(msg_cmd_vel)

#movimiento hacia la derecha cero
objetivo(goal,0,-1*ancho_derecha)
rospy.sleep((ancho_derecha)*4)

#movimiento punto de origen
objetivo(goal,0,0)
